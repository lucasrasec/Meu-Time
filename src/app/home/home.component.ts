...
export class HomeComponent {
  ...
  constructor(private authService: AuthenticationService, private router: Router, private dialog: MatDialog, private footballApi: FootballApiService) {}
  ...
  getCountries() {
    this.footballApi.getCountries().pipe(take(1)).subscribe((data: any) => {
  ...
  getSeasons() {
    this.footballApi.getSeasons().pipe(take(1)).subscribe((data: any) => {
  ...
  getLeagues(country: string) {
    this.footballApi.getLeague(country).pipe(take(1)).subscribe((data: any) => {
      this.leagues = data.response.map((result: any) => { return result.league });
      this.loadingLeagues = false
    })
  }

  getTeams(league: number, season: number) {
    this.footballApi.getTeams(league, season).pipe(take(1)).subscribe((data: any) => {
      this.leagues = data.response.map((result: any) => { return result.team });
  ...
  verifyCallTeams() {
    ...
    const leagueData = this.leagues.filter((league) => league.name === this.league)
